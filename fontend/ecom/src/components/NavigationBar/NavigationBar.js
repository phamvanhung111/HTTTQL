...
function NavigationBar({ handleSearchResult }) {
  // Receive handleSearchResult function as props
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/getByName/?name=${searchTerm}`,
        {
          headers: {
  ...
  return (
    <Navbar expand="md" className="bg-login">
      <Container>
        <Navbar.Brand href="/">
          <span
            style={{ color: "#ffffff", fontSize: "30px", fontWeight: "bolder" }}
          >
            N12
          </span>
        </Navbar.Brand>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Form inline className="d-flex ml-auto align-items-center mr-auto">
            <FormControl
              type="text"
          ...
          <Nav className="ml-auto align-items-center">
            ...
            >
              <NavDropdown.Item href="/getHistoryOrders">
                Lịch sử giao dịch
              </NavDropdown.Item>
