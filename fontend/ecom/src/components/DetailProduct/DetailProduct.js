    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                ...
                const response = await axios.get(`http://127.0.0.1:8000/api/getDetailProduct/${id}/`, {
                    headers: {
                        Authorization: `Bearer ${accessToken}`
                    }
                });
                    ...
                    <div className="col">
                        {product && (
                            <Row className='ctn-card'>
                                <Col md={4}>
                                    <Card>
                                        <Card.Img variant="top" src={`http://127.0.0.1:8000/media/${product.img}`} alt={product.name} style={{ width: '100%', height: 'auto' }} />
                                    </Card>
                                </Col>
                                <Col md={8}>
                                    <Card>
                                        <Card.Body>
                                            ...
                                            <ListGroup className="list-group-flush">
                                                ...
                                                <ListGroup.Item className='price-product'>{(product.buy_price / 1000).toFixed(3).replace(/\B(?=(\d{3})+(?!\d))/g, ".")}đ</ListGroup.Item>
                                            </ListGroup>
                                            <Form>
                                                ...
                                                <p></p>
                                                <Button variant="primary" onClick={handleBuyButtonClick}>Chọn mua</Button>
                                            </Form>
                                        </Card.Body>
                                    </Card>
                                </Col>
