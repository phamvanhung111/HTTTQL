...
def kpi(request):
    ...
    if start_date == False or end_date == False:
        ...
        dict_product = {}
        for order in orders:
            order_medicine = Order_Product.objects.filter(order_id=order.order_id)
        ...
        kpi = (sum_total_price / (num_days * 80000000)) * 100
        kpi = round(kpi, 2)
        matrix = {}
        text = ""
        if kpi < 50:
...
def updateProduct(request, id):
    try:
        ...
        medicine = Product.objects.get(product_id=id)
...
#     duong_dan_power_bi_desktop = r"D:\Pharmcy-DRF\backend\ecomapp\app\Power BI Desktop - Shortcut.lnk"
